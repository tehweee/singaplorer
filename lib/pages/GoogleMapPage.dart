...
// You will receive the itinerary details here
class GoogleMapData extends StatelessWidget {
  final Map<String, dynamic> itinerary; // Now accepts the itinerary data

  const GoogleMapData({Key? key, required this.itinerary}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // Extract hotel and attraction data from the passed itinerary
    final Map<String, dynamic>? hotel = itinerary['hotel'];
    final List<Map<String, dynamic>> attractions =
        List<Map<String, dynamic>>.from(itinerary['attractions'] ?? []);

    // Create a combined list of destinations for the map
    final List<Map<String, dynamic>> allDestinations = [];

    // Add hotel to destinations if available
    if (hotel != null &&
        hotel['latitude'] != null &&
        hotel['longitude'] != null) {
      // Safely parse latitude and longitude to double
      final double? hotelLat = double.tryParse(hotel['latitude'].toString());
      final double? hotelLng = double.tryParse(hotel['longitude'].toString());
      ...
      if (hotelLat != null && hotelLng != null) {
        allDestinations.add({
          ...
          'description': 'Your booked hotel',
          'type': 'hotel', // Add a type for distinct markers if needed
        });
      }
    }
    ...
    for (var attr in attractions) {
      if (attr['latitude'] != null && attr['longitude'] != null) {
        // Safely parse latitude and longitude to double
        final double? attrLat = double.tryParse(attr['latitude'].toString());
        final double? attrLng = double.tryParse(attr['longitude'].toString());

        if (attrLat != null && attrLng != null) {
          allDestinations.add({
            'name': attr['title'] ?? 'Attraction',
            'lat': attrLat,
            'lng': attrLng,
            'address': attr['address'] ?? 'N/A',
            'description': attr['description'] ?? 'N/A',
            'type': 'attraction', // Add a type for distinct markers if needed
          });
        }
      }
    }

    // You might want to also add arrival/departure points if they have coordinates
    // For simplicity, this example focuses on hotel and attractions.

    return MaterialApp(
      title: 'Travel Itinerary Map',
      theme: ThemeData(primarySwatch: Colors.red, fontFamily: 'Roboto'),
      home: GoogleMapPage(destinations: allDestinations), // Pass dynamic data
      debugShowCheckedModeBanner: false,
    );
  }
}
...
class _GoogleMapPageState extends State<GoogleMapPage> {
  ...
  Set<Polyline> _polylines =
      {}; // Will hold multiple polylines for detailed routes
  int _selectedDestinationIndex = 0; // Index of the destination being routed to
  List<Map<String, dynamic>> _routeSteps =
      []; // Stores detailed steps for the selected route
  Map<String, dynamic>?
  _overallRouteSummary; // Stores overall distance and duration for the selected route
  ...
  // !! Replace with your actual Google Maps API key !!
  static const String _googleMapsApiKey =
      'AIzaSyAFyK4-ffQpm-1oZJzRIR4qu0T4L8ACDwM'; // IMPORTANT: REPLACE WITH YOUR KEY

  @override
  void initState() {
    super.initState();
    _getCurrentLocation();
    // _createMarkers() will be called after current location is obtained or defaulted
    // and after destinations are set.
  }
    ...
    try {
      ...
      if (permission == PermissionStatus.granted) {
        ...
        if (mounted) {
          ...
          _createMarkers(); // Create markers after current location is known
          _getDirections(); // After getting current location, get directions to the initially selected destination
        }
      } else {
        // Handle the case where permission is not granted (e.g., show a message, use default location)
  ...
  void _useDefaultLocationAndGetDirections() {
    if (mounted) {
      ...
      _createMarkers(); // Create markers with default current location
      _getDirections(); // Get directions with default location
    }
  }
  ...
  void _createMarkers() {
    ...
    // Add markers for all destinations received
    for (int i = 0; i < widget.destinations.length; i++) {
      final destination = widget.destinations[i];
      markers.add(
        Marker(
          ...
          infoWindow: InfoWindow(
            title: destination['name'],
            snippet: destination['address'] ?? destination['description'],
          ),
          icon: BitmapDescriptor.defaultMarkerWithHue(
            destination['type'] == 'hotel'
                ? BitmapDescriptor
                      .hueAzure // A different hue for hotels
                : BitmapDescriptor.hueRed, // Red for attractions
          ),
          onTap: () {
            if (mounted) {
              setState(() {
  ...
  Future<void> _getDirections() async {
    // Ensure we have a current position and destinations to route to
    if (_currentPosition == null || widget.destinations.isEmpty) {
      if (mounted) {
        setState(() {
          _isLoadingRoute = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text(
          'Itinerary Map', // Changed title to be more general
          style: TextStyle(
            color: Colors.white,
            fontSize: 18,
            fontWeight: FontWeight.w500,
      ...
      body: Stack(
        ...
        children: [
          ...
          GoogleMap(
            onMapCreated: (GoogleMapController controller) {
              _mapController = controller;
              // Only fit to route if there are polylines
              if (_polylines.isNotEmpty) {
                _fitMapToRoute();
              } else if (widget.destinations.isNotEmpty) {
                // If no route, but destinations exist, center on the first destination/hotel
                final initialLat = widget.destinations.first['lat'];
                final initialLng = widget.destinations.first['lng'];
                _mapController?.animateCamera(
                ...
                // If no destinations, center on current location
                _mapController?.animateCamera(
                  CameraUpdate.newLatLngZoom(
                    LatLng(
                      _currentPosition!.latitude,
            ...
            // Initial camera position. Prioritize hotel, then default Singapore location.
            initialCameraPosition: CameraPosition(
              target: widget.destinations.isNotEmpty
                  ? LatLng(
                      widget.destinations.first['lat'],
                      widget.destinations.first['lng'],
                    )
                  : LatLng(
                      1.2966,
                      103.8547,
                    ), // Default to Marina Bay, Singapore
              zoom: 13.0,
            ),
            markers: _markers,
            polylines: _polylines,
                          ...
                          children: [
                            ...
                            if (_overallRouteSummary != null &&
                              ...
                              Column(
                                ...
                                children: [
                                  Text(
                                    // Use 'name' for the destination title
                                    'Route to: ${widget.destinations[_selectedDestinationIndex]['name']}',
                                    style: const TextStyle(
                                      fontSize: 16,
                                      fontWeight: FontWeight.bold,
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Text(
                                  _selectedTransportMode == 'transit'
                                      ? 'Transit Route Steps'
                                      : 'Planned Destinations', // Changed label
                                  style: const TextStyle(
                                    fontSize: 18,
                                    fontWeight: FontWeight.bold,
                                  ),
                            ...
                            if (_isLoadingRoute)
                              ...
                              // Display planned destinations for other modes
                              ListView.builder(
                                shrinkWrap:
                                    true, // Important for ListView inside SingleChildScrollView
                                physics:
                                          ...
                                          Expanded(
                                            child: Column(
                                              ...
                                              children: [
                                                ...
                                                const SizedBox(height: 2),
                                                Text(
                                                  // Show address if description is not available
                                                  destination['address'] ??
                                                      destination['description'],
                                                  style: TextStyle(
                                                    fontSize: 12,
