...
class ArrivalFlightPage extends StatefulWidget {
  @override
  _ArrivalFlightPageState createState() => _ArrivalFlightPageState();
}

class _ArrivalFlightPageState extends State<ArrivalFlightPage> {
  List<Flight> _flights = [];

  final _fromController = TextEditingController(text: 'LHR.AIRPORT');
  final _toController = TextEditingController(text: 'SIN.AIRPORT');
  final _departController = TextEditingController(text: '2025-07-25');
          ...
          Row(children: [
            Expanded(
              child: TextField(
                ...
                decoration:
                    InputDecoration(labelText: 'From (e.g. LHR.AIRPORT)'),
              ),
            ),
            SizedBox(width: 10),
          ]),
            ...
            Align(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  context,
                  MaterialPageRoute(
                    builder: (context) => ArrivalFlightDetailPage(flightDetail: flight.token),
                  ),
                );
                  showDialog(
                    context: context,
                    builder: (context) => AlertDialog(
                      ...
                      content: Text(
                          'You booked a flight with ${flight.airline} from ${flight.from} to ${flight.to}.'),
                      actions: [
                        TextButton(
