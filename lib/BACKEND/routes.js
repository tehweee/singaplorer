...
app.delete("/api/deletePlan/:id", async function (req, res) {
  let id = req.params.id;
  try {
    const user = await db.getUserByToken(global_token);
    if (!user) {
      return res.status(500).json("Error finding user");
    }
    const itinerary = await db.getItineraryById(id);
    if (!itinerary) {
      return res.status(500).json("Error finding itinerary");
    }
    if (itinerary.userId.toString() == user._id.toString()) {
      const response = await db.deleteItinerary(id)
      res.status(200).json({ message: response });
    } else {
      return res.status(403).json("User is not allowed to delete this itinerary");
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
