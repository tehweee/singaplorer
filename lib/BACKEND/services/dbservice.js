...
let db = {
  ...
  async checkoutAttraction(slug, selectedDate, pax, userid) {
      ...
      response.on('end', () => {
        try {
          ...
          const simplified = {
            ...
            totalPrice: (pax * (attractionData.representativePrice?.publicAmount ?? 0)).toString(),
            userID: userid,
            pax: pax.toString(),
  ...
  async addAttraction(
    ...
    longitude,
    totalPrice,
    userID,       // Should be a resolved ObjectId string
    pax,
    bookedDate
  ) {
    try {
      console.log("=== addAttraction() CALLED ===");
      console.log("Inputs:", {
        slug,
        title,
        pricePerPax,
      ...
      if (!mongoose.Types.ObjectId.isValid(plan_key)) {
        throw new Error(`Invalid plan_key: ${plan_key}`);
      }

      const userObjectId = new mongoose.Types.ObjectId(userID);
      const planObjectId = new mongoose.Types.ObjectId(plan_key);

      // Create attraction entry
      ...
      return `Attraction: ${title} has been added to the collection.`;
    } catch (e) {
      console.error("❌ Error in addAttraction():", e.message);
  ...
  async getLoggedIn() {
    try {
      const userWithToken = await user.findOne({ token: { $exists: true, $ne: "" } });
      if (!userWithToken) {
  ...
  async deleteItinerary(id){
    try{
      let result = await plan.findByIdAndDelete(id);
      if (!result) return "Unable to find an itinerary to delete.";
            else return "Itinerary is deleted!";
      }
    catch(e) {
        console.log(e.message);
  ...
  async updatePlanAttractions(id, newAttractionId) {
    try {
      await plan.findByIdAndUpdate(
        id,
        {
          $push: { attractionId: newAttractionId }

        }
      );
      return;
    } catch (e) {
      console.error("UpdatePlanAttractions Error:", e.message);
      throw new Error("Error at the server. Please try again later.");
    }
  },
  ...
  async addUserAIToken(conditions, updates) {
    try {
  ...
  async consumeUserAIToken(conditions, updates) {
    try {
      let result = await user.findByIdAndUpdate(conditions, updates)
      if (!result) return "Unable to find User.";
      else return "Tokens have been used!";
    }
    catch (e) {
      console.log(e.message);
      throw new Error("Error adding Tokens");
  ...
  async getManualPlanItinerary(userID) {
    try {
      let result = await plan.find({ userId: userID });
      return result;
    }
    catch (e) {
      console.log(e.message);
  ...
  async getItineraryById(id){
    try {
      let result = await plan.findById(id);
      return result
    } catch (e) {
      console.log(e.message);
      throw new Error("Error retrieving itinerary");
    }
  },
  async getArriveById(id, userID) {
    try {
        if (id == null) {
            return {
                fromCountry: "no data found",
                fromAirport: "no data found",
                departureTime: "no data found",
                arrivalTime: "no data found",
                pricePerPax: "no data found",
                totalPrice: "no data found",
                pax: "no data found",
                cabinClass: "no data found",
                token: "no data found",
                _id: "no data found",
                __v: "no data found",
                userID: userID
        ...
        const result = await arrivesg.findById(id).select();
        return result ?? {
            fromCountry: "no data found",
            fromAirport: "no data found",
            departureTime: "no data found",
        ...
        console.log(e.message);
        throw new Error("Error retrieving arrival data");
    }
},
async getDepartById(id, userID) {
  try {
    if (id == null) {
      return {
        fromCountry: "no data found",
    ...
    const result = await departsg.findById(id).select();
    return result ?? {
      fromCountry: "no data found",
      fromAirport: "no data found",
      departureTime: "no data found",
    ...
    console.log(e.message);
    throw new Error("Error retrieving departure data");
  }
},
        async getAttractionById(id){
        try{
          if (id != null){
            let result = await attraction.findById(id).select();
            return result;
          } else {
...
async getHotelById(id, userID) {
  try {
    if (id == null) {
      return {
        hotelID: "no data found",
        title: "no data found",
        ...
        totalPrice: "no data found",
        pax: "no data found",
        arrivalDate: "no data found",
        departureDate: "no data found",
        city: "no data found",
        reviewCount: "no data found",
        reviewScore: "no data found",
        _id: "no data found",
        __v: "no data found",
        userID: userID
      };
    ...
    const result = await hotel.findById(id).select();
    return result ?? {
      hotelID: "no data found",
      title: "no data found",
      address: "no data found",
    ...
    console.log(e.message);
    throw new Error("Error retrieving hotel data");
  }
},
async addSupport(userID, firstName, lastName, country, comment, date) {
  try {
    ...
    return `Support from ${firstName} ${lastName} has been added.`;
  } catch (e) {
    console.log(e.message);
    throw new Error(`Support entry from ${firstName} ${lastName} was not added.`);
  }
}
